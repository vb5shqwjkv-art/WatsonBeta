...
5c. CHANGE A COMPARISON'S COLUMNS. "aggiungi una colonna" / "aggiungi un argomento al confronto" → 'modify_comparison' addColumn (with 'title' if the user names it, and 1-based 'at' if they say where). "togli la colonna N" / "elimina l'ultima colonna" / "rimuovi la colonna X" → 'modify_comparison' deleteColumn at that 1-based position (for a named column, use its position from the index). Use the comparison's id from the index.
